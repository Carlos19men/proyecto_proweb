...
function abrirBaseDatos() {
    return new Promise((resolve, reject) => {
        // Solicitar la base de datos con la versión 3
        const request = indexedDB.open('wcData', 3);

        // Evento cuando se necesita actualizar la base de datos
        request.onupgradeneeded = function(event) {
            const db = event.target.result;
            console.log('onupgradeneeded - Se está actualizando la base de datos');
            ...
            // Crear object stores solo si no existen, sin eliminar ni modificar los existentes
        ...
        // Evento de éxito cuando se abre la base de datos correctamente
        request.onsuccess = function(event) {
            const db = event.target.result;
            console.log('Base de datos abierta exitosamente');

            // Imprimir los object stores después de abrir la base de datos
            const objectStoreNames = Array.from(db.objectStoreNames);
            console.log('Object stores después de abrir la base de datos:', objectStoreNames);

            // Verificar la existencia de los object stores y resolver la promesa
            ...
            transaction.onerror = function(event) {
                ...
                console.error('Error al verificar los object stores:', error);
                reject('Error al verificar los object stores: ' + error);
            };
        };

        // Evento de error si no se puede abrir la base de datos
...
function cargarDatos(leagueId, matchId) {
    // Intentar obtener los datos de IndexedDB primero
    Promise.all([obtenerDatos('partidos'), obtenerDatos('banderas'), obtenerDatos('confederaciones')])
        .then(([partidos, banderas, confederaciones]) => {
            if (partidos.length > 0 && banderas.length > 0 && confederaciones.length > 0) {
                console.log('Datos cargados desde IndexedDB');
                // Pasar los ids de la liga y el partido al renderizador
                renderizarPartidos({ matches: partidos }, { countries: banderas }, { leagues: confederaciones }, leagueId, matchId);
            } else {
                // Si no hay datos en IndexedDB, hacer las peticiones
                console.log('Haciendo peticiones a las APIs...');
                fetchDatosDeAPIs(leagueId, matchId);
            }
        })
        .catch(error => {
...
function fetchDatosDeAPIs(leagueId, matchId) {
    const urlPartidos = 'https://wc-qualifications-api-production.up.railway.app/api/v1/matches';
    const urlbanderas = 'https://wc-qualifications-api-production.up.railway.app/api/v1/countries';
    const urlconfederaciones = 'https://wc-qualifications-api-production.up.railway.app/api/v1/leagues';

    Promise.all([fetch(urlPartidos), fetch(urlbanderas), fetch(urlconfederaciones)])
        .then(responses => {
        ...
        .then(data => {
            ...
            const banderas = data[1].countries;
            const confederaciones = data[2].leagues;

            // Guardar datos en IndexedDB
            Promise.all([
                guardarDatos('partidos', partidos),
                guardarDatos('banderas', banderas),
                guardarDatos('confederaciones', confederaciones)
            ])
                .then(() => {
                    console.log('Datos guardados en IndexedDB correctamente.');

                    // Leer nuevamente los datos desde IndexedDB para garantizar consistencia
                    return Promise.all([
                        obtenerDatos('partidos'),
                        obtenerDatos('banderas'),
                        obtenerDatos('confederaciones')
                    ]);
                })
                .then(([partidosGuardados, banderasGuardados, confederacionesGuardadas]) => {
                    console.log('Datos cargados desde IndexedDB después de guardar.');
                    // Renderizar los partidos con los datos más recientes
                    renderizarPartidos(
                        { matches: partidosGuardados },
                        { countries: banderasGuardados },
                        { leagues: confederacionesGuardadas },
                        leagueId,
                        matchId
                    );
                })
...
function renderizarPartidos(partidos, banderas, confederaciones, leagueId, matchId) {
    const listaPartidos = partidos.matches || [];
    const listabanderas = banderas.countries || [];
    const listaconfederaciones = confederaciones.leagues || [];

    if (!Array.isArray(listaPartidos) || !Array.isArray(listabanderas) || !Array.isArray(listaconfederaciones)) {
        console.error("Los datos de partidos, países o confederaciones no están en el formato esperado");
        return;
    }

    // Filtrar el partido específico
    const partidoFiltrado = listaPartidos.find(
        partido => partido.league_id === Number(leagueId) && partido.id === Number(matchId)
    );

    if (!partidoFiltrado) {
        console.warn(`No se encontró un partido con league_id=${leagueId} y match_id=${matchId}`);
        return;
    }

    // Obtener la información de los equipos y la confederación
    const equipo1 = listabanderas.find(pais => pais.id === Number(partidoFiltrado.hometeam_id));
    const equipo2 = listabanderas.find(pais => pais.id === Number(partidoFiltrado.awayteam_id));
    const confederacion = listaconfederaciones.find(liga => liga.id === Number(leagueId));

    const partidosContainer = document.getElementById('partidos-container');
    partidosContainer.innerHTML = ''; // Limpiar el contenedor antes de agregar los nuevos datos

    const partidoHTML = `
        <section class="info-partido text-center mb-4">
            <h2 class="mb-3 subtitulos">${confederacion ? confederacion.name.es : 'Confederación no disponible'} (${partidoFiltrado.date})</h2>
            <span class="arbitro"><i class="fas fa-whistle"></i> Árbitro: ${partidoFiltrado.referee || 'Desconocido'}</span>
            <span class="hora"><i class="fas fa-clock"></i> ${partidoFiltrado.time || 'Hora no especificada'}</span>
            <span class="ronda"><i class="fas fa-trophy"></i> ${partidoFiltrado.stage || 'Ronda no especificada'}</span>
            <div class="equipos d-flex justify-content-between align-items-center">
                ...
                <div class="marcador">${partidoFiltrado.hometeam_score || 'N/A'} - ${partidoFiltrado.awayteam_score || 'N/A'} <br>
            ...
            <p class="mt-3">${partidoFiltrado.status || 'Estado desconocido'}</p>
        </section>
    `;
    partidosContainer.innerHTML = partidoHTML;

    // Llamar a las funciones para renderizar estadísticas y alineaciones
    renderizarEstadisticas(partidoFiltrado, equipo1, equipo2);
    cargarAlineaciones(partidoFiltrado, equipo1, equipo2);
}
...
document.addEventListener("DOMContentLoaded", () => {
    const { leagueId, matchId } = obtenerParametros();
    if (leagueId && matchId) {
        cargarDatos(leagueId, matchId);
    } else {
